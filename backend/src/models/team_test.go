...
func TestNewTeam(t *testing.T) {
	t.Run("Creates team with valid data", func(t *testing.T) {
		...
		team := NewTeam(name, description, ownerId)
	...
	t.Run("Creates team with empty name and description", func(t *testing.T) {
		team := NewTeam("", "", "owner123")

		assert.NotEmpty(t, team.Id)
		assert.Equal(t, "", team.Name)
		assert.Equal(t, "", team.Description)
		assert.Equal(t, TeamRoleOwner, team.Members["owner123"])
	})

	t.Run("Each team gets unique ID", func(t *testing.T) {
		team1 := NewTeam("Team 1", "Desc 1", "owner1")
		team2 := NewTeam("Team 2", "Desc 2", "owner2")
...
func TestTeam_IsMember(t *testing.T) {
	t.Run("Owner is a member", func(t *testing.T) {
		ownerId := "owner123"
		team := NewTeam("Test Team", "Description", ownerId)

		assert.True(t, team.IsMember(ownerId))
	})

	t.Run("Non-member returns false", func(t *testing.T) {
		team := NewTeam("Test Team", "Description", "owner123")
